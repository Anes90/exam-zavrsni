...
struct ZavrsniRad {
	char * _brojIndeksa;
	char * _tema;
	Poglavlje * _poglavljaRada = nullptr;
	int _trenutnoPoglavlja = 0;
	char * _datumOdbrane;
	...
	void DodajPoglavlje(const char *naslov, const char *sadrzaj) {
		if (_poglavljaRada == nullptr) {
			_poglavljaRada = new Poglavlje;
			_poglavljaRada->Unos(naslov, sadrzaj);
			_trenutnoPoglavlja++;
		}
		else {
			for (int i = 0; i < _trenutnoPoglavlja; i++) {
				if (strcmp(_poglavljaRada[i]._naslov, naslov) == 0) {
					int vel = strlen(_poglavljaRada[i]._sadrzaj) + strlen(sadrzaj) + 1;
					char * temp = new char[vel];
					...
					strcpy_s(_poglavljaRada[i]._sadrzaj, vel, temp);
			...
			for (int i = 0; i < _trenutnoPoglavlja; i++) {
				temp[i].Unos(_poglavljaRada[i]._naslov, _poglavljaRada[i]._sadrzaj);
			}
			for (int i = 0; i < _trenutnoPoglavlja; i++) {
			...
			for (int i = 0; i < _trenutnoPoglavlja; i++) {
				_poglavljaRada[i].Unos(temp[i]._naslov, temp[i]._sadrzaj);
			}
			_poglavljaRada[_trenutnoPoglavlja].Unos(naslov, sadrzaj);
			for (int i = 0; i < _trenutnoPoglavlja; i++) {
				temp[i].Dealociraj();
			}
			_trenutnoPoglavlja++;
...
struct Nastavnik {
	...
	bool DodajZavrsniRad(ZavrsniRad zavrsniRad) {
		if (_teme[max_zavrsnih - 1] != nullptr)
			return false;
		if (_teme[0] == nullptr) {
			_teme[0] = new ZavrsniRad;
			_teme[0]->Unos(zavrsniRad._brojIndeksa, zavrsniRad._tema);
			return true;
		}
		int brojac = 0;
		bool provjera = false;
		do {
			if (strcmp(_teme[brojac]->_brojIndeksa, zavrsniRad._brojIndeksa) == 0 || strcmp(_teme[brojac]->_tema, zavrsniRad._tema) == 0)
				provjera = true;
			brojac++;
		} while (_teme[brojac] != nullptr);
		if (provjera == true) {
			return false;
		}
		_teme[brojac] = new ZavrsniRad;
		_teme[brojac]->Unos(zavrsniRad._brojIndeksa, zavrsniRad._tema);
		return true;
	}
	ZavrsniRad * ZakaziOdbranuRada(const char * brojIndeksa, const char * datum) {
		...
		do {
			if (strcmp(_teme[brojac]->_brojIndeksa, brojIndeksa) == 0) {
				if (_teme[brojac]->_trenutnoPoglavlja < 3)
					return nullptr;
				for (int i = 0; i < _teme[brojac]->_trenutnoPoglavlja; i++) {
					if (strlen(_teme[brojac]->_poglavljaRada[i]._sadrzaj) < 30)
						return nullptr;
				}
				if (_teme[brojac]->_konacnaOcjena < 6)
					return nullptr;
				return _teme[brojac];
			}
			brojac++;
